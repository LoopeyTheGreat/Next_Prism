# PhotoPrism Proxy Dockerfile
# Lightweight SSH proxy for executing PhotoPrism commands in Docker Swarm
#
# This proxy accepts SSH connections from Next_Prism and executes whitelisted
# PhotoPrism commands via docker exec on the PhotoPrism container.
#
...
LABEL description="Next_Prism PhotoPrism Proxy - SSH gateway for PhotoPrism commands"
LABEL service="photoprism-proxy"
...
RUN mkdir -p /home/proxyuser/.ssh && \
    echo 'command="/usr/local/bin/validate_command.sh photoprism $SSH_ORIGINAL_COMMAND",no-port-forwarding,no-X11-forwarding,no-agent-forwarding,no-pty' > /home/proxyuser/.ssh/authorized_keys_template
...
echo "Starting PhotoPrism Proxy..."

# Set target container name from environment
export PHOTOPRISM_CONTAINER="${PHOTOPRISM_CONTAINER:-photoprism}"
echo "Target PhotoPrism container: $PHOTOPRISM_CONTAINER"

# Setup authorized_keys from secret or environment
if [ -f /run/secrets/photoprism_proxy_pubkey ]; then
    echo "Loading public key from Docker secret..."
    PUBKEY=$(cat /run/secrets/photoprism_proxy_pubkey)
